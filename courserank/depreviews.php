...
$start_from = ($page-1) * 5;
$sql = "SELECT * FROM reviews WHERE Deptt='$dep' GROUP BY CourseNumber DESC";
...
-->            
                <form class="form-horizontal" action="searchresult.php" method="post">
                     ...
                     <input type="hidden" name="dep" value="<?php echo $row["Deptt"]; ?>">   
                      <input type="hidden" name="code" value="<?php echo $row["CourseNumber"]; ?>">   
                      <input type="submit" name ="goto" value="<?php 
                      echo $row["Deptt"]." ".$row["CourseNumber"]; 
                      ?>" class="button"> 
...
<br>
            ...
            <?php
                ...
                <?php
                $sql1 = "SELECT COUNT(ID) FROM reviews WHERE Deptt='$dep'";
$rs_result1 = mysql_query ($sql);
...
$total_pages = ceil($total_records / 5);
  ...
  if (mysql_num_rows($rs_result)==0){
      echo<<<_END
               Sorry, no reivews have been added till now for $dep Department for any semester.
                    <br>
                    Add one <a href="addreview.php">here</a>.
_END;
  }
