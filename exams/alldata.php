...
if ($start<0) $start = 0;
$SQL = "SELECT id,prof_name,course,examtype,yearofexam FROM exam_file_data WHERE department='$department' order by 'id'";
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
    ...
    $responce->rows[$i]['cell']=array($row['prof_name'],$row['course'],$row['examtype'],$row['yearofexam'],'');
    $i++;
} 
